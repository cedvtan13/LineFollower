 ...
 *   100% → PWM_MAX
 */
static uint16_t SpeedToPWM(int16_t pct)
{
    if (pct <= 0)   return 0;
    if (pct > 100)  pct = 100;
    return (uint16_t)(MOTOR_DEADBAND + (pct * (PWM_MAX - MOTOR_DEADBAND)) / 100);
}
...
void Motor_SetSpeeds(int16_t leftPct, int16_t rightPct)
{
    ...
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, SpeedToPWM(leftPct  < 0 ? -leftPct  : leftPct));
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, SpeedToPWM(rightPct < 0 ? -rightPct : rightPct));
}
